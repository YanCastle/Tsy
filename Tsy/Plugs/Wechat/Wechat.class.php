...
class Wechat {
    ...
    public function replyKf($Account=''){
        return $this->response($Account,self::MSG_TYPE_DKF);
    }
    /**
     * 回复视频消息
    ...
    /**
     * 多客服转发
     * @param bool $KfAccount
     * @return bool
     */
    function kf_transfer_customer_service($KfAccount=false){
        $data =[
            'ToUserName'=>$this->data['FromUserName'],
            'FromUserName'=>$this->data['ToUserName'],
            'CreateTime'=>time(),
     ...
     * @return string
     */
     static function data2xml($xml, $data, $item = 'item') {
    ...
    private static  function transfer_customer_service($Account=false){
        if(is_string($Account)&&strpos('@',$Account)>-1){
            //转发到指定客服
            return [];
        }else{
            //转发到多客服
            return [
                'TransInfo'=>['KfAccount'=>$Account]
            ];
        }
    }
